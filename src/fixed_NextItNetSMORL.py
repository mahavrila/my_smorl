...
import setproctitle
setproctitle.setproctitle('NextItNetSMORL')
import os
import random
import numpy as np
...
class NextItNetSMORL(nn.Module):
    def __init__(self, hidden_size, item_num, state_size, dilations, div_embedding_matrix,
                 discount, smorl_loss_mult, smorl_weights, novelty_rewards_dict, device):
        super(NextItNetSMORL, self).__init__()
    ...
    def forward_next_states(self, next_states, len_next_states):
        # Supervised Head used to generate targetQs_ and targetQs_s for MORL head
        emb_next = self.item_embeddings(next_states)
        mask_next = torch.ne(next_states, self.item_num).float().unsqueeze(-1)
        emb_next *= mask_next
        conv_out_next = emb_next
        for cnn in self.cnns:
            conv_out_next = cnn(conv_out_next)
            conv_out_next *= mask_next
        state_hidden_next = extract_axis_1(conv_out_next, len_next_states - 1)
...
class Args:
    ...
    results_path = f'{div4rec}/{dataset}_results/NextItNet_RL/'
...
if __name__ == '__main__':
    ...
    for smorl_weights in args.smorl_weights_set:
        file_name = 'nextitnet_smorl{}_acc{}_div{}_nov{}.txt'.format(
        ...
        sys.stdout.flush()
        writer = SummaryWriter(log_dir=f"runs/{args.dataset}/nextitnet_{int(smorl_weights[0])}{int(smorl_weights[1])}{int(smorl_weights[2])}")
        print("I'm starting nextitnet_smorl{}_acc{}_div{}_nov{}_weighted_q_vals".format(
            str(args.smorl_loss_mult).replace('.', ''),
            smorl_weights[0],
            smorl_weights[1],
            smorl_weights[2]
        ...
        params1 = list(nextItNet1.parameters())
        optimizer1 = torch.optim.Adam([param for param in params1 if param.requires_grad], lr=args.lr)

        params2 = list(nextItNet2.parameters())
        optimizer2 = torch.optim.Adam([param for param in params2 if param.requires_grad], lr=args.lr)

        nextItNet1.to(device)
        nextItNet2.to(device)
        ...
        for epoch in range(0, args.epochs):
            ...
            for state, len_state, action, next_state, len_next_state, is_buy, is_done in train_loader:
                ...
                if pointer == 0:
                    main_QN = nextItNet1
                    target_QN = nextItNet2
                    optimizer = optimizer1
                    model_name = 'NextItNet SMORL 1'
                else:
                    main_QN = nextItNet2
                    target_QN = nextItNet1
                    optimizer = optimizer2
                    model_name = 'NextItNet SMORL 2'
                ...
                supervised_out, morl_loss = main_QN(
                ...
                loss = supervised_loss + morl_loss
                ...
                if total_step % 200 == 0:
                    ...
                    print('Supervised loss is %.3f, MORL loss is %.3f' % (supervised_loss.item(), morl_loss))
                ...
                if total_step % args.eval_iter == 0:
                    #torch.save(nextItNet1.state_dict(), args.models_path + 'nextitnet_morl{}{}{}_{}iter.pth'.format(
                    #    smorl_weights[0],
                    #    smorl_weights[1],
                    #    smorl_weights[2],
                    #    total_step))
                    ...
                    val_acc_target, predictions_target = evaluate(main_QN, args, 'val', state_size, item_num, device, args.div_emb_matrix, skip=args.skip, writer=writer, step=total_step, RL=args.rl)
                    val_acc_target, predictions_target = evaluate(main_QN, args, 'test', state_size, item_num, device, args.div_emb_matrix, skip=args.skip, writer=writer, step=total_step, RL=args.rl)

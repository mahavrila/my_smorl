...
class Args:
    ...
    results_path = f'{div4rec}/{dataset}_results/Caser_RL_0.75/'
    os.makedirs(results_path, exist_ok=True)
    results_to_file = True
    smorl_weights_set = [torch.Tensor([1.0, 1.0, 1.0]), torch.Tensor([0.0, 1.0, 1.0])]
...
if __name__ == '__main__':
    ...
    for smorl_weights in args.smorl_weights_set:
        ...
        sys.stdout.flush()
        writer = SummaryWriter(log_dir=f"runs/{args.dataset}/caser_{int(smorl_weights[0])}{int(smorl_weights[1])}{int(smorl_weights[2])}")

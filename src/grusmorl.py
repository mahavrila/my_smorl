...
import setproctitle
setproctitle.setproctitle('GRUSMORL')
import os
import random
import numpy as np
...
class GRUSMORL(nn.Module):
    def __init__(self, hidden_size, item_num, state_size, div_embedding_matrix,
                 discount, smorl_loss_mult, smorl_weights, novelty_rewards_dict, device, gru_layers=1):
        super(GRUSMORL, self).__init__()
    ...
    def forward(self, states, len_states, actions, acc_rewards, target_Qs, target_Qs_s, is_done):
        ...
        emb_packed, state_hidden = self.gru(emb_packed)
        state_hidden = state_hidden.view(-1, state_hidden.shape[2])
    ...
    def forward_next_states(self, next_states, len_next_states):
        ...
        emb_packed_next = torch.nn.utils.rnn.pack_padded_sequence(emb_next, len_next_states,
                                                                  batch_first=True, enforce_sorted=False)
        emb_packed_next, state_hidden_next = self.gru(emb_packed_next)
        state_hidden_next = state_hidden_next.view(-1, state_hidden_next.shape[2])
        ...
        for epoch in range(0, args.epochs):
            ...
            for state, len_state, action, next_state, len_next_state, is_buy, is_done in train_loader:
                ...
                if pointer == 0:
                    ...
                    model_name = 'GRU SMORL 1'
                else:
                    main_QN = gru2
                    target_QN = gru1
                    optimizer = optimizer2
                    model_name = 'GRU SMORL 2'

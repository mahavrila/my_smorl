...
class Args:
    ...
    results_path = f'{div4rec}/{dataset}_results/fix_NextItNet_rl/'
    os.makedirs(results_path, exist_ok=True)
    
    results_to_file = True
    smorl_weights_set = [torch.Tensor([1.0, 1.0, 1.0]), torch.Tensor([1.0, 0.0, 0.0]), torch.Tensor([0.0, 0.0, 0.0])]
    smorl_loss_mult = 1
    discount = 0.5
        ...
        for epoch in range(0, args.epochs):
            ...
            for state, len_state, action, next_state, len_next_state, is_buy, is_done in train_loader:
                ...
                if total_step % args.eval_iter == 0:
                    ...
                    val_acc_target, predictions_target = evaluate(main_QN, args, 'val', state_size, item_num, device, args.div_emb_matrix, skip=args.skip, step=total_step, RL=args.rl)
                    #val_acc_target, predictions_target = evaluate(main_QN, args, 'test', state_size, item_num, device, args.div_emb_matrix, skip=args.skip, step=total_step, RL=args.rl)
                    '''

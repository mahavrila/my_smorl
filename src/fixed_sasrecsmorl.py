...
class SASRecSMORL(nn.Module):
    ...
    def forward(self, states, len_states, actions, acc_rewards, target_Qs, target_Qs_s, is_done):
        ...
        acc_rewards = self.get_acc_rewards(states, supervised_preds)
...
if __name__ == '__main__':
    ...
    device = set_device(1)
    print('Using {} For Training'.format(torch.cuda.get_device_name(1)))
    for smorl_weights in args.smorl_weights_set:
        ...
        sys.stdout.flush()
        writer = SummaryWriter(log_dir=f"runs/{args.dataset}/sasrec_{int(smorl_weights[0])}{int(smorl_weights[1])}{int(smorl_weights[2])}")
